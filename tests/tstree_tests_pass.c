...
char *test_insert()
{
    ...
    mu_assert(node != NULL, "Failed to insert the node to tst.");

    node = TSTree_insert(node, bdata(&test2), blength(&test2), value2);
    mu_assert(node != NULL, "Failed to insert the second node to tst.");

    node = TSTree_insert(node, bdata(&test3), blength(&test3), reverse);
    mu_assert(node != NULL, "Failed to insert the reverse node to tst.");

    node = TSTree_insert(node, bdata(&test4), blength(&test4), value4);
    mu_assert(node != NULL, "Failed to insert the forth node to tst.");
...
char *test_search_exact()
{
    ...
    mu_assert(res == valueA, "Got the wrong value back.");
...
char *test_search_prefix()
{
    ...
    mu_assert(res == value4, "Got the wrong value4 for the prefix of 1.");
